...
const {newsData} = storeToRefs(teams())
...
const updateData = ref<TeamsType>({name:'',content:'',image:'',_id:'',job:''})
...
<template>
  ...
  <teams-modal :show="showModal" :showTeams="updateData" @submit="submit"  :imgLink="updateData.image" @closeModal="closeModal"></teams-modal>
    <div class="flex items-center justify-between">
      <h1 class="text-4xl text-primary">Teams</h1>
      <base-button type="submit" customBg="bg-green-600" class="mt-4 text-center hover:bg-green-700 duration-300 transition-all" @click="showInfo({name:'',content:'',image:'',_id:'',job:''})">Create</base-button>
    </div>
      ...
      <div class="col-span-1" v-for="team in info" :key="team.name">
        <div class="card flex items-center flex-col shadow-md rounded-md pb-4 pt-6 px-3 hover:shadow-inner transition-all duration-600 cursor-pointer relative" @click="showInfo(team)">
          ...
          <img :src="team.image" v-if="team.image" class="w-48 h-48 object-fill rounded-full" alt="">
            ...
            <h3 class="text-2xl mt-2 text-primary">{{team.job}}</h3>
